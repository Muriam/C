...
int rk_readkey(enum keys *k){ // функция для считывания клавиши
	char c[2];
	*k=undefined;
	int j=0,f=2;
	while(1) {
		read(1,&c,2);
		switch(j) {
			case 0: switch(c[0]) { // если буквы английского алфавита или Enter то смотрим только первый символ и возвращаем аналог из перечисления
				case 'l': *k=l; break;
				case 's': *k=s; break;
				case 'r': *k=r; break;
				case 't': *k=t; break;
				...
				if(c[1]=='[') {j++; continue;} // если вторым символом [, то считываем ещё два символа
				break;
			case 1: switch(c[0]) { // рассматриваем спец.клавиши
					case 'A': *k=up; break;
					case 'B': *k=down; break;
